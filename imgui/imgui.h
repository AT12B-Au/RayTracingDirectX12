...
namespace ImGui
{
    ...
    IMGUI_API float         GetSCREEN_WIDTH();                           // get current window width (shortcut for GetWindowSize().x)
    IMGUI_API float         GetSCREEN_HEIGHT();                          // get current window height (shortcut for GetWindowSize().y)
...
struct ImFont
{
    ...
    const char*                 GetDebug12Name() const                { return ConfigData ? ConfigData->Name : "<unknown>"; }
