...
void Ball::Init(UINT version)
{
	...
	else if (version == 1)
	{
		...
		//モデルロード
		{
			m_AiScene = aiImportFile("Asset\\model\\Moon.fbx", aiProcessPreset_TargetRealtime_MaxQuality | aiProcess_ConvertToLeftHanded);
			assert(m_AiScene);
		}
...
#pragma omp parallel for
				for (UINT m = 0; m < m_AiScene->mNumMeshes; m++)
				{
					...
					InstanceDesc[m].InstanceContributionToHitGroupIndex = BALL_HIT_GROUP;		//ヒットグループnum
...
#pragma omp parallel for
			for (int i = 0; i < m_InitObjectNum; i++)
			{
				...
				m_Position12 = XMFLOAT3(0.0f, 0.0f, ((FLOOR_NUM / FLOOR_LENGTH) * -FLOOR_INTERVAL + FLOOR_INTERVAL) / 2.0f - 4.0f);
