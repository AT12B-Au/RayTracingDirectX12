...
void Floor::Init(UINT version)
{
	...
	else if (version == 1)
	{
		...
		//モデルロード
		{
			m_AiScene = aiImportFile("Asset\\model\\Floor.fbx", aiProcessPreset_TargetRealtime_MaxQuality | aiProcess_ConvertToLeftHanded);
			assert(m_AiScene);
		}
...
#pragma omp parallel for
				for (UINT m = 0; m < m_AiScene->mNumMeshes; m++)
				{
					...
					InstanceDesc[m].InstanceContributionToHitGroupIndex = FLOOR_HIT_GROUP;		//ヒットグループnum
...
void Floor::UnInit(UINT version)
{
	if (version == 0)
	{
...
void Floor::Update(UINT version)
{
	if (version == 0)
	{
...
#pragma omp parallel for
			for (int i = 0; i < m_InitObjectNum; i++)
			{
				...
				m_Position12.x = 0.0f;
				m_Position12.y = -2.0f;
				m_Position12.z = 0.0f + column * -FLOOR_INTERVAL;

				if (i / FLOOR_LENGTH != static_cast<double>(i) / FLOOR_LENGTH)	//i / FLOOR_LENGTH != 整数
				{
					if (i % 2 == 0)
					{
						m_Position12.x = m_Position12.x + interval;
						m_Position12.y = -2.0f;
						m_Position12.z = 0.0f + column * -FLOOR_INTERVAL;
				...
				m_Scale12 = XMFLOAT3(1.0f, 0.5f, 1.0f);
...
#pragma omp parallel for
				for (UINT m = 0; m < m_AiScene->mNumMeshes; m++)
				{
					...
					//BLASとアタッチ
					InstanceDesc[m].AccelerationStructure = m_BLASBuffers[m].GetResultBuffer()->GetGPUVirtualAddress();
					InstanceDesc[m].InstanceMask = 0xFF;
