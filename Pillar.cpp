...
void Pillar::Init(UINT version)
{
	...
	else if (version == 1)
	{
		...
		//モデルロード
		{
			m_AiScene = aiImportFile("Asset\\model\\Pillar.fbx", aiProcessPreset_TargetRealtime_MaxQuality | aiProcess_ConvertToLeftHanded);
			assert(m_AiScene);
		}
...
#pragma omp parallel for
				for (UINT m = 0; m < m_AiScene->mNumMeshes; m++)
				{
					...
					InstanceDesc[m].InstanceContributionToHitGroupIndex = PILLAR_HIT_GROUP;		//ヒットグループnum
...
void Pillar::UnInit(UINT version)
{
	if (version == 0)
	{
...
void Pillar::Update(UINT version)
{
	if (version == 0)
	{
...
#pragma omp parallel for
			for (int i = 0; i < m_InitObjectNum; i++)
			{
				...
				m_Position12 = XMFLOAT3(0.0f,-1.0f,0.0f);
				m_Scale12 = XMFLOAT3(5.0f, 5.0f, 5.0f);

				if (i % 2 == 0)
					m_Position12.x = -FLOOR_LENGTH + 1.0f;
				else
					m_Position12.x = FLOOR_LENGTH - 1.0f;

				if (i >= 2)
					m_Position12.z = (FLOOR_NUM / FLOOR_LENGTH) * -FLOOR_INTERVAL + FLOOR_INTERVAL;
