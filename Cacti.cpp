...
void Cacti::Init(UINT version)
{
	...
	else if (version == 1)
	{
		...
		//モデルロード
		{
			m_AiScene = aiImportFile("Asset\\model\\Cacti.fbx", aiProcessPreset_TargetRealtime_MaxQuality | aiProcess_ConvertToLeftHanded);
			assert(m_AiScene);
		}
...
#pragma omp parallel for
			for (UINT m = 0; m < m_AiScene->mNumMeshes; m++)
			{
				...
				UINT IndexNum = mesh->mNumFaces * mesh->mFaces[mesh->mMaterialIndex].mNumIndices;
...
#pragma omp parallel for
				for (UINT m = 0; m < m_AiScene->mNumMeshes; m++)
				{
					...
					InstanceDesc[m].InstanceContributionToHitGroupIndex = CACTI_HIT_GROUP;		//ヒットグループnum
...
void Cacti::UnInit(UINT version)
{
	if (version == 0)
	{
...
void Cacti::Update(UINT version)
{
	if (version == 0)
	{
...
#pragma omp parallel for
			for (int i = 0; i < m_InitObjectNum; i++)
			{
				...
				m_Position12 = XMFLOAT3(-2.0f, -0.4f, ((FLOOR_NUM / FLOOR_LENGTH) * -FLOOR_INTERVAL + FLOOR_INTERVAL) / 2.0f);

				m_Scale12 = XMFLOAT3(0.1f, 0.1f, 0.1f);

				m_Position12.x += 2.0f * i;
